...
### 5. 이진파일(.uasset) 병합 규칙
`.uasset` 파일은 병합이 불가능하기 때문에, 다음과 같은 규칙을 지켜주세요. 필요한 경우 

* **하나의 BP는 한 명이 수정:** 수정 예정인 BP는 미리 팀원들에게 고지해 주세요. 필요한 경우 Git LFS 파일 잠금을 활용해 주세요.
* **병합 요청 시 BP 수정사항 기록:** BP는 병합 시 반드시 한 쪽의 BP를 덮어씌어야 하기 때문에, 병합 요청에 BP 수정사항을 상세히 기록해 주세요.
* **복잡한 로직은 C++클래스에서 작성:** `.cpp`, `.h` 파일은 비교가 쉽고 동시 작업에 제한이 없습니다. 동시 수정이 필요한 경우 소스코드 위주로 수정해 주세요.
